...
curl -L -ku ${USERPASS} -H "Content-Type: application/json" -H "Accept: application/json" -d '{"ResetType": "On"}' -X POST https://${BMC_ENDPOINT}/redfish/v1/Systems/1/Actions/ComputerSystem.Reset
if [ $? -eq 0 ]; then
  exit 0
else
  exit 1
