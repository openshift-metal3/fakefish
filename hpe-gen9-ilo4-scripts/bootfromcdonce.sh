...
curl --globoff  -L -w "%{http_code} %{url_effective}\\n"  -ku ${USERPASS}  -H "Content-Type: application/json" -H "Accept: application/json" -d '{"Boot":{"BootSourceOverrideEnabled": "Once", "BootSourceOverrideTarget": "Cd"}}' -X PATCH https://${BMC_ENDPOINT}/redfish/v1/Systems/1
if [ $? -eq 0 ]; then
  exit 0
else
  exit 1
